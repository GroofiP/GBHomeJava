...
function handleClick(clickedButtonEvent) {
    ...
    // const cardNode = ;
    const cardNone = clickedButtonEvent.target.parentNode
    // 3. создайте литерал объекта со следующими свойствами:
    const card = {
        wrap: cardNone, // здесь элемент с классом .product
        img: cardNone.querySelector('img'), // здесь картинка внутри .product
        productName: cardNone.querySelector('.productName'), // здесь .productName, который внутри .product
        button: cardNone.querySelector("button") // здесь button, который внутри .product
    };

    // 4. получаем текст на кнопке, которая внутри .product
    
    let text_button = clickedButtonEvent.target.innerText
...
function showMoreText(card) {
    ...
    const text = "lorem lorem lorem10"
